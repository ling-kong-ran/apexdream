...
#[inline(never)]
fn check_process(process_name: String) -> Option<fn(ProcessId, &mut Runtime) -> bool> {
    if process_name.contains("r5apex.exe") {
    ...
    while !signal_exit {
        // Main loop to find game processes
        {
            ...
            for (pid, process) in sys.processes() {
                ...
                if let Some(run) = check_process(process.name().to_string_lossy().to_string()) {
                    rt.signal = 0;
                    let process_id_instance: ProcessId = unsafe { std::mem::transmute(process_id.as_u32()) };
                    if run(process_id_instance, &mut rt) {
                        return;
                    }
                    rt.process = None;
